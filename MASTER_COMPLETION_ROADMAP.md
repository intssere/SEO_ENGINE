...
| P8.8 | Progressive policy-authorized low-risk execution | SPECIFICATION/REVIEW ACTIVE — issue #406; initial class fixed to Shopify product SEO `meta_description`; staged work packages W01–W10 define policy grant/evaluation, P9.7 proposal materialization, distinct policy authorization, durable reservation/idempotency, control integration, policy-aware preflight/apply, audit projection, shadow certification and separately authorized canary activation; implementation/schema/worker/live activation remain blocked |
