...
| P9.6 | worker observability and pause/kill controls | NEXT SAFE BOUNDARY — deterministic/default-off worker state, health/observability, pause and kill-switch architecture over supplied/fake state only; no worker/scheduler/retry-loop activation, durable queue mutation, provider/crawl request, persistence or publication on generic continuation |
