...
test("P8.8 W07 migration source is additive transactional dispatch-only DDL", async () => {
  ...
  assert.match(source, /p8-8-w07-policy-dispatch-v1/);
  assert.match(source, /p8-8-w07-policy-dispatch-event-v1/);
  assert.match(source, /execution_provenance = 'policy_single_action_apply'/);
  assert.match(source, /forward_attempt_count IN \(0,1\)/);
  assert.match(source, /rollback_attempt_count IN \(0,1\)/);
  ...
  assert.doesNotMatch(
    source,
    /access_token|refresh_token|authorization_header|raw_response|approval_actor|APPLY_AND_VERIFY_TASK54|EXECUTE_AND_ROLLBACK_TASK53/i,
  );
});

test("P8.8 W07 migration applies only to dedicated ephemeral 41-table W05 baseline", async (t) => {
  ...
  assert.deepEqual(tables.map((row) => row.table_name), [...TABLES].sort());
  ...
  const names = dispatchIndexes.map((row) => row.indexname);
  assert.ok(names.includes("ux_policy_mutation_dispatches_active_site"));
  assert.ok(names.includes("ux_policy_mutation_dispatches_active_target"));
  assert.ok(names.includes("idx_policy_mutation_dispatches_site_history"));

  const eventIndexes = await sql.unsafe<{ indexname: string }[]>(
    "SELECT indexname FROM pg_indexes WHERE schemaname='public' "
      + "AND tablename='policy_mutation_dispatch_events' ORDER BY indexname",
  );
  assert.ok(
    eventIndexes.some(
      (row) => row.indexname === "idx_policy_mutation_dispatch_events_history",
    ),
  );
