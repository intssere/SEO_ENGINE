...
test("P8.8 W07 migration source is additive, transactional and contains no DML", async () => {
  ...
  assert.match(source, /rollback_attempt_count <= forward_attempt_count/);
  assert.match(source, /state IN \(/);
  assert.match(source, /'reserved_prewrite'/);
  assert.match(source, /'dispatch_started'/);
  ...
  assert.doesNotMatch(
    source,
    /proposal_text|before_value|after_value|current_value|proposed_value|raw_response|access_token|refresh_token|oauth|secret|approval_actor|APPLY_AND_VERIFY_TASK54/i,
  );
});
...
test("P8.8 W07 migration applies only to the dedicated ephemeral W05 41-table baseline", async (t) => {
  ...
  assert.deepEqual(
    actual.map((row) => row.table_name).sort(),
    [...TABLES].sort(),
  );
  ...
  for (const expected of [
    "ux_policy_mutation_dispatches_active_site",
    "ux_policy_mutation_dispatches_active_target",
    "idx_policy_mutation_dispatches_site_history",
  ]) {
    assert.ok(dispatchIndexes.some((row) => row.indexname === expected));
  }
