...
test("P8.8 W07 migration source is additive, transactional and contains no DML", async () => {
  ...
  assert.equal((source.match(/\bCREATE\s+(?:UNIQUE\s+)?INDEX\s+/gi) ?? []).length, 5);
  ...
  assert.match(source, /resource_gid ~ '\^gid:\/\/shopify\/Product/);
  assert.match(source, /field = 'meta_description'/);
  assert.doesNotMatch(
    source,
    /before_value|after_value|proposal_text|proposed_value|current_value|raw_response|access_token|refresh_token|oauth|secret|approval_actor|APPLY_AND_VERIFY_TASK54|EXECUTE_AND_ROLLBACK_TASK53/i,
  );
});
...
test("P8.8 W07 migration applies only to the dedicated ephemeral W05 41-table baseline", async (t) => {
  ...
  for (const name of [
    ...
    "idx_policy_mutation_dispatches_target_history",
  ]) {
    assert.ok(dispatchIndexes.some((row) => row.indexname === name), name);
  }
