...
CREATE TABLE policy_mutation_dispatches (
  ...
  w06_preflight_id text NOT NULL UNIQUE,
  w06_preflight_fingerprint char(64) NOT NULL UNIQUE,
  ...
  rollback_request_fingerprint char(64),
  rollback_response_fingerprint char(64),
  final_closure_reason text,
  reserved_at timestamptz NOT NULL,
  dispatch_started_at timestamptz,
  forward_result_at timestamptz,
  ...
  CHECK (
    rollback_request_fingerprint IS NULL
    OR rollback_request_fingerprint ~ '^[0-9a-f]{64}$'
  ),
  CHECK (
    rollback_response_fingerprint IS NULL
    OR rollback_response_fingerprint ~ '^[0-9a-f]{64}$'
  ),
  CHECK (
    state IN (
      'reserved_prewrite',
  ...
  CHECK (
    (state = 'reserved_prewrite' AND forward_attempt_count = 0 AND rollback_attempt_count = 0)
    OR
    (state = 'cancelled_before_dispatch' AND forward_attempt_count = 0 AND rollback_attempt_count = 0)
    OR
    (
      state NOT IN ('reserved_prewrite','cancelled_before_dispatch')
...
CREATE TABLE policy_mutation_dispatch_events (
  ...
  CHECK (
    ...
    OR
    (from_revision IS NOT NULL AND from_revision >= 1 AND from_state IS NOT NULL
...
CREATE INDEX idx_policy_mutation_dispatches_target_history
  ON policy_mutation_dispatches (
    site_id,
    resource_gid,
    field,
