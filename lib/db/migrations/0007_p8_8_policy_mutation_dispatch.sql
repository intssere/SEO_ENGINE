...
CREATE TABLE policy_mutation_dispatches (
  ...
  policy_execution_id text NOT NULL UNIQUE,
  ...
  w03_expires_at timestamptz NOT NULL,
  w06_preflight_expires_at timestamptz NOT NULL,
  ...
  forward_response_at timestamptz,
  ...
  CHECK (w03_expires_at > reserved_at),
  CHECK (w06_preflight_expires_at > reserved_at),
  CHECK (
    state IN (
      'reserved_prewrite',
      'dispatch_started',
  ...
  CHECK (
    (state IN ('rollback_started','rollback_verification_pending','rollback_verified_closed')
      AND rollback_attempt_count = 1)
    OR
    (state NOT IN ('rollback_started','rollback_verification_pending','rollback_verified_closed')
...
CREATE TABLE policy_mutation_dispatch_events (
  ...
  CHECK (
    (prior_revision IS NULL AND prior_state IS NULL AND next_revision = 1)
    OR
    (prior_revision IS NOT NULL AND prior_state IS NOT NULL
      AND next_revision = prior_revision + 1)
  )
);
