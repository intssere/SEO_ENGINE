...
Default next safe boundary: **P10.7 — Impact workspace v2**. Generic continuation may define deterministic/read-only workspace projection and frontend presentation over exact P10.1–P10.6 supplied/certified artifacts. It must preserve chronology, association, windows, experiment structure, outcome arithmetic and calibration signals as distinct non-causal layers; it must not query live providers, mutate Production data/schema, create reward/ranking/policy updates, start P9.8 autonomous mutation, execute Task #51/#53/#54, deploy or publish.
