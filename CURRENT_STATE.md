...
Default next safe boundary: **P11.5 — accessibility WCAG 2.2 AA certification**. Generic continuation may perform deterministic source/static review, local/synthetic browser accessibility testing and bounded engineering-source remediation for semantic structure, keyboard/focus behavior, accessible names/labels, ARIA use, landmarks/headings, reflow, contrast/visual states and WCAG 2.2 AA criteria. It does not authorize production crawling/scanning, provider/public-site mutation, Production DB/storage mutation, secret/runtime/deployment changes, scheduler/worker activation, Task #51/#53/#54 execution, P9.8 implementation/activation, deployment or publication.
