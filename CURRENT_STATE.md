...
- **P12.1 — NOT READY / SAFE ENGINEERING REMAINS:** primary navigation still contains `Learning` as `status: "planned"`; primary routes must be audited so no placeholder/demo/synthetic fixture is presented as live production state.
...
**Default next safe boundary: P12.1 — primary-surface placeholder / production-state audit and bounded remediation.** Generic continuation may audit and remediate frontend/navigation/source/tests so every primary route is a deliberate v1 surface with honest production/disconnected/empty/loading/error/stale states and synthetic fixtures cannot be mistaken for live data. It does **not** authorize live provider/OAuth activity, production crawling, Production DB/storage mutation, evidence persistence activation, provider/public-site writes, scheduler/worker activation, autonomous mutation, credential changes, destructive retention, deployment or publication.
