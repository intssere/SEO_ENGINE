...
- The P12.2 default-off Diamond Shelf crawl execution/persistence bridge is complete. Its sitemap/robots/page/clock/persistence capabilities remain injected only; `networkReady`, `liveExecutionAuthorized`, `persistenceReady` and `persistenceAuthorized` are call-boundary gates and are not enabled by merge, sync or generic continuation. Existing P2 authorization objects remain closed.
