...
- P11.1 performance hardening is deterministic/offline. Build budgets are explicit reviewed ceilings; do not raise or disable a budget merely to make a regression pass without a scoped reviewed change. Synthetic-local timing profiles are not production RUM, Core Web Vitals certification, load testing, ranking signals or a user-experience score.
- P11.1 browser profiling must remain on the local synthetic Playwright boundary unless a later task explicitly authorizes another target. External-origin requests and unmocked API calls remain failures.
- Generic continuation into P11.2 may perform deterministic/offline source/config/dependency security review and bounded test-based hardening for auth, CSRF, SSRF, CSP, headers, secret handling and supply chain. It does not authorize intrusive production scanning, exploitation, secret retrieval/rotation, provider credential/scope changes, Production DB mutation, deployment/runtime configuration changes, P9.8 implementation/activation, Task #51/#53/#54 execution, deployment or publication.
