...
- P8.8 W02 governed-proposal materialization is pure/default-off and must rebuild exact P9.7 recommendation lineage plus exact P6.6 preview lineage before trusting supplied artifacts. Detached or tampered recommendation/preview objects must fail closed.
- W02 remains closed to one canonical `proposal_review`, one field total, one changed field exactly `meta_description`, and an explicit caller-supplied fingerprinted Diamond Shelf Shopify Product target binding. Never infer/discover a Product GID or URL, broaden to collections/titles/media/content, or query Shopify from W02.
- W02 before/after values are byte-for-byte P6.6 current/proposed values. Do not trim, rewrite, normalize, generate, template, enrich, or substitute copy in the materialization bridge.
- W02 output lifecycle is `materialized_unpersisted`. Its proposal/materialization IDs and fingerprints are deterministic in-memory identities only and never imply ProposalRecord persistence, approval, policy authorization, execution authorization, public-write permission, or live provider state.
- W02 may expose only static W01 materialized facts. It must not fabricate bounded-pilot authorization, provider-observed current state, deployment/concurrency/cooldown/quota/manual-intervention/uncertain-write/rollback/control facts.
- W02 performs no DB/network/provider/runtime work and must not be wired to a route, startup hook, scheduler, worker, persistence adapter, provider client or Task #51/#53/#54 path from generic continuation. W03 and later P8.8 work packages require separate explicit authorization.
- P8.8 W03 policy authorization is pure/default-off and must canonically rebuild exact W01 admission plus exact W02 materialization before trust. Detached/tampered W01/W02 artifacts or any shared policy/recommendation/proposal/target/before/after mismatch must fail closed.
- W03 provenance is exactly `policy_authorization`; never fabricate or accept human approval ID/decision/actor/timestamp, an approvals row, the Task #51 `controlled_execution_foundation_v1` envelope, or a Task #54 human confirmation string as W03 authority.
- Before W04 exists, W03 reservation binding is synthetic/non-durable only: `reserved_prewrite`, version `p8-8-w03-synthetic-reservation-v1`, source `synthetic_test`, durable=false. W03 must never claim or create a durable reservation/action row.
- W03 `policyActionId` and `policyAuthorizationId` are deterministic artifact identities only. `persistedActionId` remains null and `persistedActionCreated=false`; `providerWriteAllowed`, `providerDispatchAuthorized`, `publicSiteWrites`, `automaticTransition` and `dispatchEligible` remain false.
- W03 caller-supplied issuance must be canonical, TTL 1–15 minutes, and expiry bounded by the existing W01 evaluation/grant windows. W03 never reads wall-clock time or renews itself.
- W03 performs no DB/network/provider/runtime work and must not be wired to a route, startup hook, scheduler, worker, persistence adapter, provider client, approvals/action persistence, or Task #51/#53/#54 path from generic continuation. W04 and later P8.8 work packages require separate explicit authorization.
- P8.8 W04 durable reservation engineering uses a dedicated `policy_mutation_reservations` namespace. Never reuse human `actions` or generic worker `jobs` as policy reservation authority.
- W04 durable identity must derive from canonical W01/W02 lineage and must match the exact synthetic/non-durable reservation ID/fingerprint precommitted through unchanged W03 v1. Arbitrary caller-selected reservation identities fail closed.
- W04 store construction requires an explicit database URL. Do not add a `DATABASE_URL` fallback, route/startup binding, scheduler, worker, provider client, or automatic Production migration path.
- W04 active/blocking reservation states are `authorized`, `claimed`, `manual_intervention`; terminal/non-active states are `consumed`, `released`, `expired`. Only stale unclaimed `authorized` rows may be opportunistically expired. Never auto-expire `claimed` or `manual_intervention`.
- W04 database constraints enforce one active/blocking reservation per site and one per exact target field for the initial `shopify.product.seo.meta_description` class. Do not broaden provider/domain/resource/action/field/scope constraints without a separately reviewed migration.
- W04 exact replay may return the existing durable row, but identity collision, site concurrency conflict, target conflict, or uncertain database state must fail closed. Do not add automatic retries around uncertain transactions.
- A valid W03+W04 pairing means policy-aware control eligibility only. It never implies provider dispatch, public-site write, Task #51/#53/#54 authority, worker activation, or policy activation.
- Migration `0005_p8_8_policy_mutation_reservations.sql` is engineering source only until explicit Production DDL authorization names the exact canonical SHA/tree and migration checksum. Generic continuation must never apply it to Production or create Production reservation rows.
- P8.8 W05 durable mutation-control engineering uses only `policy_mutation_control_state`, `policy_mutation_control_events` and `policy_mutation_claims` as its durable policy-control namespace. Never reuse or mutate human `approvals`/`actions`, Task #54 deployment/verification rows, or generic worker `jobs` as W05 authority.
- W05 migration 0006 must not create a default `running` row. Missing durable mutation-control state fails closed; Production control initialization requires separate explicit authority.
- W05 preserves P9.6 precedence `kill > drain > pause > running`, but mutation admission is stricter: only durable `running` may permit a new W05 claim. Pause, drain and kill always block new forward mutation.
- Every W05 operation needing both control and reservation authority must lock the site control row before the W04 reservation row. Exact expected control revision/fingerprint and exact W03/W04 pairing are mandatory; stale or detached control/authorization fails closed.
- W05 claim identity is immutable and deterministic over the exact W03/W04 pair plus accepted control revision/fingerprint. A new claim must atomically insert exactly one W05 claim and transition W04 `authorized -> claimed`; exact replay may return that claim, while a claimed W04 row without the exact claim is uncertain state.
- W05 pause/drain/kill may release only an exact paired unclaimed `authorized` reservation. Never automatically release or expire `claimed` or `manual_intervention`; later no-dispatch proof or safety closure belongs to separately certified W06/W07 semantics.
- W05 `killed` is latched and cannot ordinary-resume. Resume from pause/drained/draining requires zero unresolved claimed/manual-intervention blocker and never creates a reservation or claim.
- W05 must keep new forward mutation distinct from mandatory provider/storefront verification, bounded rollback or manual-intervention safety closure after a possible side effect. Control closure never grants a new provider write.
- W05 store construction requires an explicit database URL and W05 migration/concurrency tests use only dedicated `P8_8_W05_EPHEMERAL_DATABASE_URL`; never fall back to generic `DATABASE_URL`.
- Migration `0006_p8_8_policy_mutation_controls.sql` is engineering source only until a separate Production DDL authorization names exact canonical SHA/tree and checksum. Generic continuation must never apply migrations 0005/0006, initialize Production control state or create Production claims.
- W05 provider-dispatch/public-write/Task #51/#53/#54/worker/scheduler/automatic-transition markers remain false. W06 and later P8.8 work packages remain separately gated; W05 completion does not authorize policy-aware provider preflight/apply, autonomous activation, deployment or publication.
- P8.8 W06 policy-aware preflight is mutation-free. W06 readiness never grants provider dispatch/write authority; it requires exact W01–W05 lineage, exact same running control epoch, DB-clock authorization freshness, and byte-exact W02-domain provider before-state proof. Task #53 normalized state fingerprints never substitute for W02 policy-state equality.
- W06 no-dispatch proof is narrow: it proves only that the exact W06 path did not begin provider mutation dispatch. W06 never performs rollback and never releases/consumes a claim itself.
- W07 specification/review remains separate from implementation and live activation. W07 must use a dedicated policy execution provenance and must never fabricate Task #51 approval or Task #53/#54 confirmation/deployment authority.
- Any future W07 forward mutation requires a durable two-phase policy dispatch fence before provider mutation. W04 must remain `claimed` throughout nonterminal forward verification/rollback work; do not set `consumed` at dispatch start.
- Future W07 `claimed -> released` is allowed only with exact proven no-dispatch closure; `claimed -> consumed` only after a spent forward attempt reaches a safe terminal no-write/live/rollback-verified state; uncertainty must become `manual_intervention`.
- W07 policy mutation must preserve exact W02 byte-for-byte after/before values. Do not reuse Task #53 mutation normalization for the policy path.
- Future W07 provider writes are at-most-once: no automatic forward retry after `dispatch_started`, and no second rollback after `rollback_started`. Verification reads may retry only under bounded propagation semantics.
- A future W07 retained live result requires exact W02-domain provider after-state plus independent provider and storefront verification. A provider mutation receipt alone is never success.
- Pause/drain/kill block new W07 forward dispatch. If side effect may already have occurred, verification/rollback safety closure may continue; kill after a nonterminal observed-after state routes toward the already-bounded single rollback path.
- A future W07 implementation must remain default-off and require a dedicated policy mutation execution gate in addition to the global public-write gate. W10, not W07 merge, owns live Stage 1 activation.
- Future migration `0007_p8_8_policy_mutation_dispatch.sql` and all W07 Production DDL/DML/provider access remain separately authorized. Generic `continue` may advance W07 documentation/review/specification only, not implementation, migration application, provider mutation, rollback mutation, runtime activation, deployment or publication.
- P8.8 W08 autonomous-audit artifacts are deterministic read-only projections only. The audit root is the exact W03 `policyActionId`; shared site, target, field, fingerprints, recommendation lineage, timestamps or ordering must never manufacture policy-action lineage.
- W08 must preserve policy-specific W01–W07 IDs/fingerprints and must not fabricate human approval IDs, persisted human action IDs, deployment IDs or Task #54 confirmation. W03 policy authorization, W04 reservation, W05 claim/control, W06 preflight and W07 dispatch remain distinct authority namespaces.
- W08 exact replay may collapse, but conflicting replay for the same source identity must fail closed. Later timestamps or higher row revisions never silently overwrite conflicting evidence.
- W08 chronology/order/hash chains are serialization and tamper evidence only. They do not establish current provider state, execution authority, success, priority, causality, recommendation quality or policy fitness.
- W08 must project W07 append-only dispatch events with exact dispatch ID, revision/state and event fingerprint. It must not invent W04 reservation transitions where migration 0005 lacks authoritative event timestamps, and it must not attach site-level W05 control events by temporal/site proximity.
- W08 unavailable, uncertain, provider/storefront mismatch and manual-intervention evidence must remain explicit and must never be promoted to verified success. Mutation receipts or one-sided provider/storefront evidence are insufficient.
- Initial W08 engineering requires no migration 0008 and no database/provider/runtime binding. Generic `continue` may advance W08 documentation/review/specification only; W08 E1–E5 implementation, Production reads/writes, provider access, scheduler/worker/policy activation, deployment/publication, W09 and W10 remain separately gated.
- P8.8 W09 Stage 0 shadow artifacts are zero-write decision/certification evidence only. A shadow `admit` must never be promoted to W03 policy authorization, W04 reservation, W05 claim, W06 provider preflight, W07 dispatch, human approval, Task #54 confirmation or live provider-write permission.
- W09 must call and integrity-check the certified W01 evaluator; do not duplicate or fork policy rules into a second shadow policy engine.
- W09-A engineering is pure/default-off and caller-supplied only: no Production DB read/write, persistence, provider/public-site read/write, migration, runtime route/startup binding, scheduler/worker activation, gate/config change, deployment or publication.
- A later W09-B real Stage 0 run requires separate authorization if direct Production reads or persistence are needed. Real-candidate labels alone do not prove provenance; exact source IDs/fingerprints must bind any claimed real snapshot.
- W09 exact replay may collapse; conflicting replay for the same source identity must fail closed. Input order, recency, favorable decisions or newer policy versions must never silently resolve a conflict.
- Human-governed comparison in W09 is descriptive only and requires exact target identity/fingerprints. Same site, similar copy, recommendation similarity, timestamps or ordering must never manufacture comparability or imply correctness/causality.
- The certified P8.7 collection `meta_description` pilot is historical human-path evidence only; it is not exact-target comparable to the initial W09 Shopify Product `meta_description` class.
- W09 must certify the exact W01 rejection matrix including pause/drain/kill, quota, cooldown, concurrency, unresolved manual intervention, uncertain provider write and rollback failure, without consuming quota/cooldown/concurrency or mutating control.
- W09 Stage 0 completion does not activate W10. W09-A engineering is a pure/default-off caller-supplied module only; it must remain runtime-unbound and must reject `production_read_snapshot` acquisition inside the engineering module.
- W09-A source fingerprints bind provenance/source identity plus the exact W01 grant/candidate/reference/expiry snapshot; caller-supplied W01 evaluation must be recomputed with the certified W01 evaluator and match exactly before any shadow decision is trusted.
- W09-A sessions are bounded to 1,000 supplied items, collapse exact replay, fail closed on conflicting source replay or mixed policy identity, and use canonical ordering only for deterministic serialization.
- W09-A human comparison accepts only an independently integrity-verified P8.6 ledger with exact Product mutation class/resource GID/target URL/field/before/after fingerprint binding. Collection/P8.7 evidence remains not comparable to the initial Product class.
- W09-A comparison and session summaries remain descriptive only: do not compute policy/human accuracy, policy fitness, live-readiness scores, causal impact, confidence, quality rankings or activation recommendations.
- A merged W09-A implementation still authorizes no W09-B real Stage 0 acquisition. Generic `continue` may advance W09-B documentation/review only; direct Production DB/provider reads, Stage 0 persistence and W10 Stage 1 remain separately gated.
- W09-B's first real-evidence design is DB-read-only, provider-network-free and non-persistent. A future run must use a named Production database/role plus one frozen SELECT-only query set; generic `DATABASE_URL`, ad hoc SQL and broad table dumps are forbidden.
- W09-B must keep raw Production acquisition provenance separate from W09-A. W09-A remains DB-free and consumes evidence-complete outputs only as `supplied_real_snapshot`, linked to an immutable W09-B acquisition manifest.
- First-run W09-B caps: at most 25 final candidates and at most 100 source opportunities scanned. Exceeding a per-candidate evidence/provenance cap must produce an incomplete candidate rather than silent truncation.
- Never infer Shopify Product GID from URL or text. Never relabel crawl/page/storefront evidence as provider-authoritative before-state evidence. Missing exact Product GID or valid provider-before provenance makes the candidate incomplete.
- Absence of Production migrations/tables 0005/0006/0007 must fail closed as missing W05/W07 state. Do not apply migrations, assume control=`running`, infer zero concurrency/quota history, or synthesize mutation state during W09-B.
- W09-B first-run outputs remain outside Production. Any persistent Stage 0 evidence store, fresh Shopify/provider read, migration, runtime activation or W10 Stage 1 requires a separate review and explicit authorization.
- W09-BE1–BE5 engineering must remain synthetic/ephemeral only. BE2 is the sole DB-bound module and must require an explicit database URL; never read `process.env.DATABASE_URL`, never import the ambient `@workspace/db` pool, and never expose arbitrary caller-supplied SQL.
- W09-BE v1 has exactly 18 frozen SELECT-only query descriptors. Descriptor SQL, parameters, allowed columns, ordering, invocation caps and row caps are integrity-bound into the query-set fingerprint; ad hoc SQL is forbidden.
- W09-BE2 must use one connection and one `READ ONLY / REPEATABLE READ` transaction with bounded timeouts. No row/advisory locks, DML/DDL, migration, retry-to-another-database or persistence is allowed.
- W09-BE3 is pure/offline and may translate only integrity-valid `complete_for_w09a` records into W09-A `supplied_real_snapshot`. It must never emit `production_read_snapshot` or bypass W09-A's independent W01 recomputation.
- W09-BE provider-before authority requires an exact persisted W06 `P88W06ProviderObservation` envelope that passes W06 integrity, exact target/fingerprint/provenance and freshness checks. Labels such as `source_kind=provider`, crawler/storefront observations, confidence or matching text are never sufficient.
- W09-BE4 database tests must use `P8_8_W09B_EPHEMERAL_DATABASE_URL` and localhost/127.0.0.1 only. The adapter must not create/drop databases; only the test harness may prepare disposable local databases for schema-state cases.
- W09-BE5 must prove no provider adapter/fetch, write SQL, migration, persistence, runtime route/startup/scheduler/worker binding, Task #51/#53/#54 runtime execution or W10 activation. A merged BE1–BE5 implementation still authorizes no real Production/provider Stage 0 run.
- W09-C real Stage 0 remains a separately authorized one-shot Production DB-read-only session. Its eligible query set is exactly 18 descriptors with fingerprint `fd117eb142807ac982808bf3cd30f96009884b3c66366489875693e44103cabd`; any code/query drift invalidates the authorization packet.
- W09-C must bind exactly to site `eb1da9ee-539c-4200-8f04-f64ccaea7768` / `https://diamondshelf.us`; require `grant.siteId === acquisitionPackage.site.siteId` before translation; never infer site identity from domain text alone.
- Canonical state currently records Production at 34 tables while W09-BE2 requires W04/W05/W07 objects from migrations 0005/0006/0007. Do not open a Production connection merely to rediscover this known gap. W09-C is blocked until a separate schema-readiness review is complete.
- A future W09-C run also requires exact Production project/environment/database, exact read-only role/write-incapability proof, exact shadow-grant artifact/fingerprint and approved external evidence destination/retention. Do not inspect secrets/config to fill these fields under generic continuation.
- W09-C1 may review migration 0005/0006/0007 checksums, additive DDL, backups, rollback and post-migration verification. It must not apply Production DDL without a separate explicit authorization.
