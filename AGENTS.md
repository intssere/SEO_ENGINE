...
- P10.7 Impact workspace v2 is deterministic synthetic/read-only presentation only. It must keep P10.1 chronology, P10.2 direct association, P10.3 windows/confounders, P10.4 experiment/holdout structure, P10.5 expected-vs-actual arithmetic and P10.6 directional calibration visibly and semantically separate.
- P10.7 must have no live Impact API binding, provider/outcome loader, Production DB access, mutation hook, scheduler/worker primitive or execution control. The initial fixture must remain visibly labeled `SYNTHETIC READ-ONLY`, `NO CAUSAL VERDICT`, and `NO EXECUTION`.
- P10.7 upstream report/version/fingerprint lineage, canonical timestamps, outcome identities and calibration bindings must fail closed on malformed, duplicate, dangling or conflicting facts. Unavailable values remain unavailable.
- P10.7 UI/model must never turn signed differences or directional signals into impact score, improvement/regression, success/failure, recommendation quality, reward/penalty, model feedback, ranking weight, confidence, causal effect or rollout/retain/rollback advice.
- Generic continuation into P11.1 may define deterministic build-time/local performance budgets and bounded synthetic/local profiling only. It does not authorize live production traffic/load generation, provider calls for profiling, Production DB mutation, P9.8 implementation/activation, Task #51/#53/#54 execution, deployment or publication.
