...
- Generic continuation after P12.1 may perform only read-only preflight/runbook/dependency verification for P12.2–P12.4. A production crawl, OAuth consent, provider request, external-intelligence call, Production DB/storage mutation/persistence, scheduler/worker activation, credential change, destructive retention, provider/public-site write, Task #51/#53/#54 execution, deployment or publication requires its existing separate explicit authorization.
