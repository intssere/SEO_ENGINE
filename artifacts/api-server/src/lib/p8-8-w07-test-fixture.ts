...
export function buildP88W07TestFixture(options: {
  ...
  const scenario = buildP88W04TestScenario({
    ...
    baseTime,
    productId: "630000001",
    handle: "w07-policy-single-action",
    subjectSuffix: options.subjectSuffix ?? "w07-policy-single-action",
  ...
  const observation = buildP88W06ProviderObservation({
    ...
    rawValue: lineage.w02Materialization.before.value,
    observedBeforeFingerprint: p88W02StateFingerprint({
      target: lineage.w02Materialization.target,
      value: lineage.w02Materialization.before.value,
      purpose: "before",
    }),
  ...
  return {
    ...
    control,
    credentialBinding: {
      credentialProfileId: scenario.intentInput.w01EvaluationInput.grant.credentialProfileId,
      siteId: intent.siteId,
