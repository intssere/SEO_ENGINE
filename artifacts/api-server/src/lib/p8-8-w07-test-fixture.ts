...
export function buildP88W07TestFixture(options: {
  ...
  const scenario = buildP88W04TestScenario({
    ...
    baseTime,
    productId: options.productId ?? "630000001",
    handle: options.handle ?? "w07-single-action",
    subjectSuffix: "w07-" + (options.productId ?? "630000001"),
    currentValue: options.before ?? "Before  W07\nbytes",
    proposedValue: options.after ?? "After  W07\nbytes",
    policyStage: options.policyStage,
  });
  const intent = scenario.input.intent;
  const w03 = scenario.input.w03Authorization;
  const claimedAt = new Date(Date.parse(baseTime) + 1_000).toISOString();
  ...
  const control = initializeP88W05ControlState({
    ...
    mode: "running",
    effectiveAt: new Date(Date.parse(baseTime) - 1_000).toISOString(),
  }).state;
  ...
  const snapshot: P88W06DurableSnapshot = Object.freeze({
    ...durableState,
    databaseNow: baseTime,
    snapshotFingerprint: p88W06DurableSnapshotFingerprint(durableState),
  });
  ...
  const providerObservation = buildP88W06ProviderObservation({
    ...
    requestProvenanceFingerprint: p88W07StableHash({
      fixture: "p8.8-w07-provider-before",
    }),
    errorCategory: null,
  });
  ...
  const bundle: P88W07LineageBundle = Object.freeze({
    w06Lineage: lineage,
    w06PreSnapshot: snapshot,
    w06FinalSnapshot: snapshot,
    w06ProviderObservation: providerObservation,
    w06Preflight: preflight,
  });
