...
const Q01 = descriptor({
  ...
  sql: "SELECT table_name,column_name FROM information_schema.columns WHERE table_schema='public' AND (table_name || '.' || column_name)=ANY($1::text[]) ORDER BY table_name ASC,ordinal_position ASC LIMIT 257",
  parameterKeys: ["requiredColumns"],
  allowedTables: ["information_schema.columns"],
  allowedColumns: ["table_name", "column_name", "table_schema", "ordinal_position"],
  maxInvocations: 1,
  maxRowsPerInvocation: 256,
  maxTotalRows: 256,
