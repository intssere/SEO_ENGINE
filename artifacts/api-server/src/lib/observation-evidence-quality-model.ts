...
export const OBSERVATION_EVIDENCE_QUALITY_MODEL_ENABLED = false as const;
...
export const EVIDENCE_QUALITY_CONFLICT_INDEX_INTENTS: readonly EvidenceQualityConflictIndexIntent[] = Object.freeze([
  Object.freeze({ name: "evidence_quality_semantic_support", unique: false, fields: Object.freeze(["siteId", "canonicalOrigin", "semanticKey", "supportTier"] as const), purpose: "quality" as const }),
  Object.freeze({ name: "evidence_quality_availability_freshness", unique: false, fields: Object.freeze(["siteId", "evidenceAvailability", "freshnessState"] as const), purpose: "quality" as const }),
  Object.freeze({ name: "evidence_quality_provenance", unique: false, fields: Object.freeze(["siteId", "semanticKey", "provenanceFingerprint", "sourceKind"] as const), purpose: "provenance" as const }),
  Object.freeze({ name: "evidence_conflict_value_support", unique: false, fields: Object.freeze(["siteId", "semanticKey", "valueFingerprint", "provenanceFingerprint"] as const), purpose: "conflict" as const }),
  Object.freeze({ name: "evidence_conflict_resolution", unique: false, fields: Object.freeze(["siteId", "conflictGroupId", "resolutionState"] as const), purpose: "resolution" as const }),
]);
...
const SUPPORT_RANK: Record<EvidenceSupportTier, number> = { insufficient: 0, limited: 1, supported: 2, strong: 3, corroborated: 4 };
...
function sha256(value: unknown): string { return createHash("sha256").update(stableSerialize(value)).digest("hex"); }
function uniqueSorted(values: readonly string[]): readonly string[] { return Object.freeze([...new Set(values)].sort()); }
...
function supportTierFor(row: HistoryReadRow, freshnessState: EvidenceFreshnessState, corroboratingIndependentProvenanceCount: number): EvidenceSupportTier {
  ...
  if (row.evidenceAvailability === "available" && (row.confidence === "high" || row.confidence === "verified") && corroboratingIndependentProvenanceCount > 0) return "corroborated";
...
function assessmentFor(row: HistoryReadRow, rowById: ReadonlyMap<string, HistoryReadRow>, referenceTime: string): EvidenceQualityAssessment {
  ...
  const visibleCorroboratingRows = row.corroboratingObservationIds.map((id) => rowById.get(id)).filter((value): value is HistoryReadRow => value !== undefined);
  ...
  const corroboratingIndependentProvenanceCount = new Set(visibleCorroboratingRows.filter((value) => value.valueFingerprint === row.valueFingerprint).map((value) => value.provenance.provenanceFingerprint).filter((fingerprint) => fingerprint !== row.provenance.provenanceFingerprint)).size;
  const supportTier = supportTierFor(row, freshnessState, corroboratingIndependentProvenanceCount);
  const base = { observationId: row.observationId, semanticKey: row.semanticKey, valueFingerprint: row.valueFingerprint, sourceKind: row.provenance.sourceKind, provenanceFingerprint: row.provenance.provenanceFingerprint, evidenceAvailability: row.evidenceAvailability, unavailableEvidenceDimensions: uniqueSorted(row.unavailableEvidenceDimensions), freshnessState, confidence: row.confidence, visibleCorroboratingObservationIds, unavailableCorroboratingObservationIds, corroboratingIndependentProvenanceCount, conflictParticipantCount: row.conflictObservationIds.length, supportTier };
...
function aggregateSupportTier(assessments: readonly EvidenceQualityAssessment[]): EvidenceSupportTier {
  ...
  return assessments.reduce<EvidenceSupportTier>((best, assessment) => SUPPORT_RANK[assessment.supportTier] > SUPPORT_RANK[best] ? assessment.supportTier : best, "insufficient");
}
...
function compareValueSupport(a: ConflictValueSupportProjection, b: ConflictValueSupportProjection): number {
  ...
  for (let index = 0; index < tupleA.length; index += 1) { const delta = (tupleB[index] ?? 0) - (tupleA[index] ?? 0); if (delta !== 0) return delta; }
...
function conflictComponents(model: RetentionHistoryReadModel): readonly { ids: readonly string[]; relationFingerprints: readonly string[] }[] {
  ...
  for (const start of [...adjacency.keys()].sort()) {
    ...
    const pending = [start]; const ids: string[] = [];
    while (pending.length > 0) { const current = pending.shift(); if (!current || visited.has(current)) continue; visited.add(current); ids.push(current); for (const next of [...(adjacency.get(current) ?? [])].sort()) if (!visited.has(next)) pending.push(next); }
    const sortedIds = [...ids].sort(); const relationFingerprints: string[] = [];
    for (let leftIndex = 0; leftIndex < sortedIds.length; leftIndex += 1) for (let rightIndex = leftIndex + 1; rightIndex < sortedIds.length; rightIndex += 1) { const fingerprint = relationByPair.get(`${sortedIds[leftIndex]}:${sortedIds[rightIndex]}`); if (fingerprint) relationFingerprints.push(fingerprint); }
...
function conflictGroupFor(component: { ids: readonly string[]; relationFingerprints: readonly string[] }, rowById: ReadonlyMap<string, HistoryReadRow>, assessmentById: ReadonlyMap<string, EvidenceQualityAssessment>): ConflictGroupProjection {
  ...
  const valueSupports = valueFingerprints.map((valueFingerprint) => {
    ...
    const corroboratingRows = corroboratingIds.map((id) => rowById.get(id)).filter((value): value is HistoryReadRow => value !== undefined).filter((row) => row.valueFingerprint === valueFingerprint);
    ...
    const supportingAssessments = supportingRows.map((row) => assessmentById.get(row.observationId)).filter((value): value is EvidenceQualityAssessment => value !== undefined);
    const supportTier = aggregateSupportTier(supportingAssessments);
    const base = { valueFingerprint, conflictParticipantObservationIds: uniqueSorted(participantRows.map((row) => row.observationId)), supportingObservationIds: uniqueSorted(supportingRows.map((row) => row.observationId)), missingCorroboratingObservationIds: uniqueSorted(corroboratingIds.filter((id) => !rowById.has(id))), provenanceFingerprints: uniqueSorted(supportingRows.map((row) => row.provenance.provenanceFingerprint)), sourceFingerprints: uniqueSorted(supportingRows.map((row) => row.provenance.sourceFingerprint)), freshObservationCount: supportingAssessments.filter((assessment) => assessment.freshnessState === "fresh").length, availableEvidenceCount: supportingAssessments.filter((assessment) => assessment.evidenceAvailability === "available").length, strongOrBetterCount: supportingAssessments.filter((assessment) => SUPPORT_RANK[assessment.supportTier] >= SUPPORT_RANK.strong).length, supportTier };
  ...
  let resolutionState: ConflictResolutionState = "retain_unresolved"; let preferredValueFingerprint: string | null = null; const reasoningCodes: string[] = [];
  if (coverage === "partial_page" || valueSupports.length < 2) { resolutionState = "insufficient_evidence"; reasoningCodes.push("participant_rows_unavailable"); }
  else if (valueSupports.some((support) => support.supportTier === "insufficient")) { resolutionState = "insufficient_evidence"; reasoningCodes.push("participant_evidence_insufficient"); }
  else {
    const strongCandidates = valueSupports.filter((support) => SUPPORT_RANK[support.supportTier] >= SUPPORT_RANK.strong && support.provenanceFingerprints.length >= 2);
    if (strongCandidates.length === 1) {
      const candidate = strongCandidates[0];
      const competingBestRank = Math.max(...valueSupports.filter((support) => support.valueFingerprint !== candidate.valueFingerprint).map((support) => SUPPORT_RANK[support.supportTier]));
      if (SUPPORT_RANK[candidate.supportTier] > competingBestRank) { resolutionState = "prefer_supported"; preferredValueFingerprint = candidate.valueFingerprint; reasoningCodes.push("unique_independent_corroborated_support"); }
      else { resolutionState = "require_review"; reasoningCodes.push("multiple_strong_support_states"); }
    } else if (strongCandidates.length > 1) { resolutionState = "require_review"; reasoningCodes.push("multiple_strong_support_states"); }
    else { resolutionState = "retain_unresolved"; reasoningCodes.push("no_strict_support_dominance"); }
  }
  const groupBase = { relationFingerprints: component.relationFingerprints, observationIds: component.ids, semanticKey };
  const conflictGroupId = sha256(groupBase);
  const base = { conflictGroupId, semanticKey, observationIds: component.ids, missingObservationIds, coverage, valueSupports: Object.freeze(valueSupports), resolutionState, preferredValueFingerprint, reasoningCodes: uniqueSorted(reasoningCodes) };
  return Object.freeze({ ...base, groupFingerprint: sha256(base) });
}

export function buildEvidenceQualityConflictModel(context: EvidenceQualityConflictContext, query: EvidenceQualityConflictQuery = {}): EvidenceQualityConflictModel {
  ...
  const allAssessments = Object.freeze(context.readModel.rows.map((row) => assessmentFor(row, rowById, referenceTime)).sort((a, b) => a.observationId.localeCompare(b.observationId)));
  const assessmentById = new Map(allAssessments.map((assessment) => [assessment.observationId, assessment] as const));
  const allConflictGroups = Object.freeze(conflictComponents(context.readModel).map((component) => conflictGroupFor(component, rowById, assessmentById)).sort((a, b) => a.conflictGroupId.localeCompare(b.conflictGroupId)));
  const matchedAssessments = allAssessments.filter((assessment) => { if (normalized.supportTiers && !normalized.supportTiers.includes(assessment.supportTier)) return false; if (normalized.freshnessStates && !normalized.freshnessStates.includes(assessment.freshnessState)) return false; if (normalized.evidenceAvailability && !normalized.evidenceAvailability.includes(assessment.evidenceAvailability)) return false; return true; });
  const matchedConflictGroups = allConflictGroups.filter((group) => !normalized.conflictResolutionStates || normalized.conflictResolutionStates.includes(group.resolutionState));
  ...
  const page = Object.freeze({ offset: normalized.offset, limit: normalized.limit, returned: assessments.length, totalMatched: matchedAssessments.length, hasMore: normalized.offset + assessments.length < matchedAssessments.length });
  const queryBase = { supportTiers: normalized.supportTiers ?? null, freshnessStates: normalized.freshnessStates ?? null, evidenceAvailability: normalized.evidenceAvailability ?? null, conflictResolutionStates: normalized.conflictResolutionStates ?? null, offset: normalized.offset, limit: normalized.limit };
  const queryFingerprint = sha256(queryBase); const indexIntentFingerprint = sha256(EVIDENCE_QUALITY_CONFLICT_INDEX_INTENTS);
  const base = { recordType: "observation_evidence_quality_conflict_model" as const, schemaVersion: P3_5_SCHEMA_VERSION, siteId: context.readModel.siteId, canonicalOrigin: context.readModel.canonicalOrigin, readModelFingerprint: context.readModel.resultFingerprint, referenceTime, queryFingerprint, assessments, conflictGroups: Object.freeze(matchedConflictGroups), page, indexIntentFingerprint, authorization: EVIDENCE_QUALITY_CONFLICT_AUTHORIZATION };
  return Object.freeze({ ...base, resultFingerprint: sha256(base) });
}

export function assertEvidenceQualityConflictModelIntegrity(model: EvidenceQualityConflictModel, context: EvidenceQualityConflictContext, query: EvidenceQualityConflictQuery = {}): void {
  if (!model || model.recordType !== "observation_evidence_quality_conflict_model" || model.schemaVersion !== P3_5_SCHEMA_VERSION) throw new Error("evidence_quality_model_invalid");
  const rebuilt = buildEvidenceQualityConflictModel(context, query);
  if (rebuilt.resultFingerprint !== model.resultFingerprint || stableSerialize(rebuilt) !== stableSerialize(model)) throw new Error("evidence_quality_model_fingerprint_mismatch");
}
